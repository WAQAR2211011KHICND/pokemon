...
describe("Testing Center Component",()=>{

    it("Should have a Default Image of Pikachu" ,async ()=>{
        const {findByAltText} = render(<NoteState><Center/></NoteState>); 
        const pikachuImage = await findByAltText("pikachu")
        expect(pikachuImage).toHaveAttribute('src',defaultPokemon.sprites.other.dream_world.front_default);
    })

    it("Checking the id, name , height, weight, type, Abilities of Pikachu", async ()=>{
        render(<NoteState><Center/></NoteState>);    
        expect(await screen.findByText('#025')).toBeInTheDocument();
        expect(await screen.findByText(/pikachu/i)).toBeInTheDocument();
        expect(await screen.findByText(/height: [0-9]+/i)).toBeInTheDocument();
        expect(await screen.findByText(/weight: [0-9]+/i)).toBeInTheDocument();
        expect(await screen.findByText(/type: [a-zA-Z]+/i)).toBeInTheDocument();
        expect(await screen.findByText(/abilities: [a-zA-Z]+/i)).toBeInTheDocument();
    })
